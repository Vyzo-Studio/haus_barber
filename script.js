...
bookingForm?.addEventListener('submit', (event) => {
  ...
  const formattedDate = year && month && day ? `${day}/${month}/${year}` : dataDesejada;
  ...
  window.open(`https://wa.me/5561995705082?text=${encodeURIComponent(message)}`, '_blank', 'noopener,noreferrer');
});
...
function renderLightboxItem(item) {
  ...
  if (lightboxCaption) {
    lightboxCaption.textContent = item.dataset.title || item.dataset.alt || '';
  }
}
...
function openLightbox(button) {
  ...
  currentLightboxIndex = Math.max(0, visibleItems.indexOf(button));
